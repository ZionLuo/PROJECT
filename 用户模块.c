...
void loadData() {
    ...
    if (fp) {
        ...
        while (fscanf(fp, "%s %s", u, p) == 2) {
            ...
            strcpy(newM->username, u); strcpy(newM->password, p);
            newM->dishList = NULL; newM->next = NULL;
    ...
    if (fp) {
        ...
        while (fscanf(fp, "%s %s", u, p) == 2) {
            ...
            strcpy(newC->username, u); strcpy(newC->password, p);
            newC->addrList = NULL; newC->next = NULL;
    ...
    if (fp) {
        ...
        while (fscanf(fp, "%s %s", u, p) == 2) {
            ...
            strcpy(newR->username, u); strcpy(newR->password, p);
            newR->next = NULL;
    ...
    if (fp) {
        OrderNode *tail = NULL;
        int oid, st; char cn[30], dn[50], addr[100];
        while (fscanf(fp, "%d %s %s %s %d", &oid, cn, dn, addr, &st) == 5) {
            OrderNode *newO = (OrderNode*)malloc(sizeof(OrderNode));
            newO->orderId = oid; strcpy(newO->customerName, cn);
            strcpy(newO->dishName, dn); strcpy(newO->address, addr);
            newO->status = st; newO->next = NULL;
...
void registerMenu() {
    ...
    if (role == 1) {
        ...
        while (p) { if (strcmp(p->username, uname) == 0) { printf("顾客已存在！\n"); system("pause"); return; } p = p->next; }
        FILE *fp = fopen("customers.txt", "a");
        fprintf(fp, "%s %s\n", uname, pwd); fclose(fp);
        CustomerNode *newC = (CustomerNode*)malloc(sizeof(CustomerNode));
        strcpy(newC->username, uname); strcpy(newC->password, pwd);
        newC->addrList = NULL; newC->next = NULL;
        if (!customerHead) customerHead = newC;
        else { p = customerHead; while (p->next) p = p->next; p->next = newC; }
        printf("顾客注册成功！\n"); system("pause");
    } else if (role == 2) {
        MerchantNode *p = merchantHead;
        while (p) { if (strcmp(p->username, uname) == 0) { printf("商家已存在！\n"); system("pause"); return; } p = p->next; }
        FILE *fp = fopen("merchants.txt", "a");
        fprintf(fp, "%s %s\n", uname, pwd); fclose(fp);
        MerchantNode *newM = (MerchantNode*)malloc(sizeof(MerchantNode));
        strcpy(newM->username, uname); strcpy(newM->password, pwd);
        newM->dishList = NULL; newM->next = NULL;
        if (!merchantHead) merchantHead = newM;
        else { p = merchantHead; while (p->next) p = p->next; p->next = newM; }
        printf("商家注册成功！\n"); system("pause");
    } else if (role == 3) {
        RiderNode *p = riderHead;
        while (p) { if (strcmp(p->username, uname) == 0) { printf("骑手已存在！\n"); system("pause"); return; } p = p->next; }
        FILE *fp = fopen("riders.txt", "a");
        fprintf(fp, "%s %s\n", uname, pwd); fclose(fp);
        RiderNode *newR = (RiderNode*)malloc(sizeof(RiderNode));
        strcpy(newR->username, uname); strcpy(newR->password, pwd);
        newR->next = NULL;
        if (!riderHead) riderHead = newR;
        else { p = riderHead; while (p->next) p = p->next; p->next = newR; }
        printf("骑手注册成功！\n"); system("pause");
    }
}
...
void merchantMenu(MerchantNode *merchant) {
    ...
    while (1) {
        ...
        printf("\n===== 商家后台 (%s) =====\n", merchant->username);
        printf("1. 添加菜品\n2. 修改菜品\n3. 查看我的菜品\n0. 退出登录\n");
        printf("请选择: "); scanf("%d", &choice); getchar();
        ...
        if (choice == 1) {
            ...
            int id; char name[50]; double price;
            printf("输入要修改的菜品ID: "); scanf("%d", &id);
            DishNode *p = merchant->dishList;
            while (p != NULL && p->id != id) p = p->next;
            if (p) {
                printf("输入新名称: "); scanf("%s", name);
                printf("输入新价格: "); scanf("%lf", &price);
                strcpy(p->name, name); p->price = price;
                printf("修改成功！\n"); system("pause");
            } else { printf("未找到该菜品！\n"); system("pause"); }
        } else if (choice == 3) {
            DishNode *p = merchant->dishList;
...
void riderMenu(RiderNode *rider) {
    ...
    while (1) {
        ...
        if (choice == 1) {
            OrderNode *p = orderHead; int found = 0;
...
void customerMenu(CustomerNode *customer) {
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            while (m) {
                DishNode *d = m->dishList;
                while (d) { if (strstr(d->name, keyword)) printf("找到: [%s] %s (￥%.2f)\n", m->username, d->name, d->price); d = d->next; }
                m = m->next;
            }
            system("pause");
        } else if (choice == 3) {
            int addrChoice;
            printf("1. 添加地址 2. 查看地址: "); scanf("%d", &addrChoice);
            if (addrChoice == 1) {
                ...
                while (p) { printf("地址%d: %s\n", p->id, p->detail); p = p->next; }
                system("pause");
            }
        } else if (choice == 4) {
            ...
            char newPwd[30];
            printf("输入新用户名: "); scanf("%s", customer->username);
            printf("输入新密码(需包含字母/数字/符号中的至少两种): "); scanf("%s", newPwd);
